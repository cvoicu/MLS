		...
		try {
			...
			if(checkYes(scan)){
				...
				while (true) {
					Thread.sleep(500);
					/*if( i== 2 ) {
						System.out.println("TEST!!! send request for topic: topic; offset:0");
						
						ZMQ.Context context = ZMQ.context(1);
						ZMQ.Socket sock = context.socket(ZMQ.REQ);
						
						sock.connect("tcp://172.31.26.142:7004");
						
						sock.send("topic");
						System.out.println("the topic was sent and received" + new String(sock.recvStr()));
						//send the number of the message:
						sock.send("0");
						System.out.println("the message number was sent and received" + new String(sock.recvStr()));
						sock.send("0");
						
						String exist =sock.recvStr();
						
						System.out.println("The messsage exist?: " + exist);
						
						if(exist.compareTo("OK")==0) {
							sock.send("OK");
							String length = sock.recvStr();
							sock.send("OK");
							ByteBuffer msg = ByteBuffer.allocateDirect(Integer.parseInt(length));
							sock.recvZeroCopy(msg, Integer.parseInt(length), 0);
							int j=0;
							String s = "";
							while(j<msg.capacity()) {
								s = s + (char)msg.get(j);
								j++;
							}
							System.out.println("TEST!!! message received: start->|" + s +"|<-fin");
						}
					}
					i++;*/
				}
