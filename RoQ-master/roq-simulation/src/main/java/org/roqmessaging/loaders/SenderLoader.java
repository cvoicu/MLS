...
public class SenderLoader extends TimerTask implements IStoppable {
	 ...
	 * @param topic 
	 */
	public SenderLoader(int rate, int payload, String configServerAddress, String queue, int topic) {
		super();
		this.topic = topic;
		this.rate = rate;
		...
		int sizeMD = (Long.toString(System.currentTimeMillis()) + " ").getBytes().length + "topic".getBytes().length;
	...
	public void run() {
		...
		String pl = "Aceste este un messaj";
		while(this.sentMsg<this.rate){
			publisher.sendMessage(("topic"+1234).getBytes(), pl.getBytes());
			this.sentMsg++;
		}
	}
