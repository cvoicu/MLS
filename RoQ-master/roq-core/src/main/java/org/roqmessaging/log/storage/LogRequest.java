	...
	public void run() {
		...
		while (true) {
			...
			if (poller.pollin(0)) {
				...
				logger.debug("$$$$$$ TEST 3" );
				String topic = logRequestSocket.recvStr();
				logRequestSocket.send("OK");
				long msgNo = Long.valueOf(logRequestSocket.recvStr());
				logRequestSocket.send("OK");
				long offset = Long.valueOf(logRequestSocket.recvStr());
