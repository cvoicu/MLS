...
import org.roqmessaging.log.reliability.Node;
...
public class MessagePersistence {
	...
	public MessagePersistence(String logAddress, String seeds, String propertyFile) {
		...
		start.bind("inproc://"+nodeName+"start");
		
		if(useLog) {
			storage = new MessageHandler(properties, context, nodeName, logAddress);
			node = new Node(properties, context, nodeName, replicationFactor, nodeAddress, configPort, dataPort, this.seeds, storage);
