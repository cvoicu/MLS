 ...
 * monitor management.
 * 
 * @authors sskhiri, Cristian Voicu
 */
public class HostConfigManager implements Runnable, IStoppable {
	...
	//Address of the all others exchanges to be used as seed.
	...
	public void run() {
		...
		int clientSock = items.register(this.clientReqSocket);

		// 2. Start the main run of the monitor
		while (this.running) {
			items.poll(100);
			if (items.pollin(clientSock)) { // Comes from a client
				...
				case RoQConstant.CONFIG_CREATE_QUEUE:
					...
					if (info.length == 2) {
						...
						boolean xChangeOK = startNewExchangeProcess(qName, this.qMonitorMap.get(qName),	this.qMonitorStatMap.get(qName));
				...
				case RoQConstant.CONFIG_CREATE_EXCHANGE:
					...
					if (info.length == 4) {
						...
						logger.error("The create new exchange does not contain 5 parts: ID, Qname, monitor, monitor host, logAddress");
						this.clientReqSocket.send(
								(Integer.toString(RoQConstant.CONFIG_CREATE_QUEUE_FAIL) ).getBytes(), 0);
					}
					break;
	...
	private boolean startNewExchangeProcess(String qName, String monitorAddress, String monitorStatAddress) {
		...
		// choose the port and create the list of seeds.
		int logPort = frontPort + 10000;
		seeds = seeds+ip+":"+logPort+"#";
		globalConfigSocket.send(new Integer(RoQConstant.GET_SEEDS).toString()+", BruceLee beats VanDame");
		seeds = seeds+ globalConfigSocket.recvStr();
		...
		try {
			ProcessBuilder pb = new ProcessBuilder("java", "-Djava.library.path="
					...
					new Integer(backPort).toString(), monitorAddress, monitorStatAddress, ip+":"+logPort, seeds, propertyFile);
			logger.debug("Starting: " + pb.command());
			final Process process = pb.start();
			pipe(process.getErrorStream(), System.err);
			pipe(process.getInputStream(), System.out);
