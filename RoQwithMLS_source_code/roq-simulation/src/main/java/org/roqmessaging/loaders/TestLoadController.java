...
public class TestLoadController implements IStoppable {
		...
		try {
			...
			while (toCreate > 0) {
				for (int i = 0; (i < spawnRate) || (lastPosition<this.publisherConnections.size()); i++, lastPosition++) {
					...
					int testRate = generator.nextInt(11)+5;
					int testPayload = generator.nextInt(11)+34;
					SenderLoader loader = new SenderLoader(350/*testRatecvoicu:this.testDesc.getRate()*/, 38/*testPayload cvoicuthis.testDesc.getPayload()*/, this.gcmAddress, this.qName);
	...
	private void createAndAttachSubscriber(int maxSub) {
		for (int i = 0; i < maxSub; i++) {
			IRoQSubscriberConnection subscriberConnection = conFactory.createRoQSubscriberConnection(qName, "top1");
			// Open the connection to the logical queue
			subscriberConnection.open();
			// Register a message listener
			IRoQSubscriber subs = new IRoQSubscriber() {
				private int count = 0;
				private int sizeMD = (Long.toString(System.currentTimeMillis()) + " ").getBytes().length + "top1".getBytes().length;
