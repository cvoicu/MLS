...
public class LogicalQFactory implements IRoQLogicalQueueFactory {
	...
	public MyLook launchHost = new MyLook(true);
	...
	public void launchNewHost() {
		if (launchHost.getLook()) {
			launchHost.setLook(false);
			RequestHost rHost = new RequestHost(launchHost, this.globalConfigReq);
			Thread t = new Thread(rHost);
			t.start();
		}
	}
}
