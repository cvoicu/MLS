...
import org.roqmessaging.scaling.policy.SimpleScalingPolicy;
...
public class ScalingProcess extends KPISubscriber {
	...
	SimpleScalingPolicy simpleScalingPolicy = null;
	/**
	...
	public ScalingProcess(String globalConfiguration, String qName, int listnerPort) {
		...
		GCMaddress = globalConfiguration;
		this.qName = qName;
		...
		simpleScalingPolicy = new SimpleScalingPolicy(GCMaddress);
	}
	...
	private void initialization() {
		...
		//cvoicu: wait for queue to be initialized.
		BSONObject myBsonObject = new BasicBSONObject();
		myBsonObject.put("CMD", 1234500);
		myBsonObject.put("QName", qName);
		
		byte[] encoded = BSON.encode(myBsonObject);

		int myCheck = 1234502;
		while (myCheck != 1234501) {
			// 2. Send the request
			...
			BSONObject response = BSON.decode(bres);
			myCheck = (int) response.get("RESULT");
			
			logger.info("cvoicu: mycheck= "+ myCheck + " cvoicu: comment= "+ response.get("COMMENT"));
			try {
				Thread.sleep(2000);
			} catch (InterruptedException e) {
	...
	public void setAutoScalingConfig() {
		...
		LogicalQScalingRule qRule = new LogicalQScalingRule(10000, 1500000);
	...
	private AutoScalingConfig getAutoScalingConfig() {
		try {
			...
			// 3. Check the result
			byte[] bres = requestSocket.recv(0);
			logger.info("cvoicu: reponse from mgmController= " + ((bres == null) ? "null" : bres.length ));
			logger.info("cvoicu: unserialize conf: "+ serializer.unserializeConfig(bres));
		...
		if (this.scalingConfig != null) {
			...
			case 23:
				...
				if (overloaded) {
					...
					simpleScalingPolicy.scaleOut(context, this.qName);
